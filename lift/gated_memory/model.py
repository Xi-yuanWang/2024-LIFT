...
class GMLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(config, layer_idx) for layer_idx in range(start_idx)] + [
                GMLlamaDecoderLayer(config, layer_idx) for layer_idx in range(start_idx, config.num_hidden_layers-end_idx, 1)] + [
                    LlamaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers-end_idx, config.num_hidden_layers, 1)]
        )
