...
class GLUGate(nn.Module):
    def __init__(self, num_layer: int, res: bool, scaling: float, num_key_value_groups: int, num_key_value_heads: int, *linargs, tailsigmoid=True, **linkwargs):
        ...
        self.proj = nn.Sequential(GroupedLinear(self.num_key_value_heads, self.head_dim, middim, bias=False), nn.SiLU(inplace=True), GroupedLinear(self.num_key_value_heads, middim, 1, bias=False)) 
...
class GMLlamaAttention(LlamaAttention):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.mem_proj = GLU(3, True, config.num_key_value_heads, self.head_dim, self.head_dim, bias=True)
        self.gate_proj = GLUGate(2, True, self.scaling, self.num_key_value_groups, config.num_key_value_heads, self.head_dim, self.head_dim, bias=True)
