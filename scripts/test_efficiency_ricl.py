...
class NIAHArgs:
    ...
    long_output: bool = field(default=False, metadata={'help': "Test long output."})
    no_truncate: bool = field(default=False)
...
def main():
    ...
    for sample in tqdm(all_inputs[num_resumed:], desc="Evaluating", initial=num_resumed, total=len(all_inputs)):
        ...
        with torch.no_grad():
            ...
            if input_ids.shape[-1] > model_max_length and not niah_args.no_truncate:
            ...
            output_ids = model.generate(
                input_ids=input_ids,
                attention_mask=attention_mask,
                max_new_tokens=sample['length'] if niah_args.long_output else 50,
                do_sample=False,
        ...
        assert output_ids[input_ids.shape[-1]:].shape[-1] == (sample['length'] if niah_args.long_output else 50)
        print(f"Avg Time: {total_time / (sample['length'] if niah_args.long_output else 50)} ms")
        ...
        result = {
            ...
            'time': total_time / (sample['length'] if niah_args.long_output else 50)
        }
