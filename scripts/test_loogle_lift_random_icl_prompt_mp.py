...
class TestArguments:
    ...
    mix_training: bool = field(default=True, metadata={'help': "Mix QA and context segments in the 2nd stage."})
    num_process: int = field(default=1, metadata={'help': "The number of processes to use."})


def format_qa_pair(tokenizer: PreTrainedTokenizer, qa_pair: Dict, title: str, full_context: str, model_max_length: int=2048, use_cot: bool=False):
    evidence = qa_pair['S']
    question = qa_pair['Q']
    answer = qa_pair['A']
    if use_cot:
    ...
    input_ids = tokenizer(example, add_special_tokens=False)['input_ids']
...
class LooGLEDataset(ICLContextDataset):
    def __init__(self, context: str, title: str, tokenizer: PreTrainedTokenizer, model_max_length: int=4096, block_size: int=256, len_segment: int=8, len_offset: int=3, num_syn_qa: int=0, title_option: int=1, generator_name_or_path: Optional[str]=None, use_cot: bool=False, mix_training: bool=True, syn_qa_cache: Optional[Dict]=None):
        ...
        if num_syn_qa > 0:
            if syn_qa_cache is None:
                ...
                qa_pairs = np.random.choice(syn_qa_cache, num_syn_qa)
                print('!' * 10, len(qa_pairs))
                print(qa_pairs[0])
            for qa_pair in qa_pairs:
                input_ids, input_length = format_qa_pair(self.tokenizer, qa_pair, title, context, model_max_length, use_cot)
    ...
    def generate_task(self, generator: PreTrainedModel, full_context: str, context_sent: List[str], title: str, model_max_length: int, use_cot: bool=False):
        ...
        for _ in range(5):
            ...
            if evidence not in context:
                continue
            return {'S': evidence, 'Q': question, 'A': answer}
        else:
...
def prediction(data: List[Dict], training_args: TrainingArguments, lift_args: Dict, output_file: str, num_resumed: int=0, num_syn_qa: int=0, title_option: int=1, generator_name_or_path: Optional[str]=None, use_cot: bool=False, mix_training: bool=True):
    ...
    for i, sample in enumerate(tqdm.tqdm(data, desc="Sample")):
        ...
        model = LooGLEtrain(context, title, tokenizer, training_args=training_args, num_syn_qa=num_syn_qa, title_option=title_option, generator_name_or_path=generator_name_or_path, use_cot=use_cot, mix_training=mix_training, syn_qa_cache=sample.get('syn_qa', None), **lift_args)
        model.eval()
...
def prediction_wrapper(gpu_id: int, seed: int=0, **kwargs):
...
def main():
    ...
    num_process = test_args.pop('num_process')
    num_resumed = 0
    ...
    mp_input_data = [[] for _ in range(num_process)]
    for i in range(len(input_data)):
        mp_input_data[i % num_process].append(input_data[i])

    mp_output_dir = f'mp_{os.path.dirname(output_file)}'
    mp_output_filename = os.path.basename(output_file)
    mp_output_files = [os.path.join(mp_output_dir, f'{mp_output_filename}_{i}_in_{num_process}') for i in range(num_process)]
    mp_num_resumed = [0 for _ in range(num_process)]

    # print(mp_output_dir)
    # print(mp_output_filename)
    # print(mp_output_files)

    if not os.path.exists(mp_output_dir):
        os.makedirs(mp_output_dir)

    for i, output_file in enumerate(mp_output_files):
        if os.path.exists(output_file):
            if overwrite:
                os.remove(output_file)
            else:
                with open(output_file, 'r') as f:
                    mp_num_resumed[i] = len(f.readlines())
    ...
    for rank in range(num_process):
        mp_kwargs = {
            'gpu_id': rank,
            'data': mp_input_data[i],
            'training_args': training_args,
            'lift_args': lift_args,
            'output_file': mp_output_files[i],
            'num_resumed': mp_num_resumed[i],
            **test_args,
        }
        p = mp.Process(target=prediction_wrapper, kwargs=mp_kwargs)
        p.start()
        processes.append(p)
    for p in processes:
        p.join()

    mp_results = [[] for _ in range(num_process)]
    for i, mp_output_file in enumerate(mp_output_files):
        with open(mp_output_file, 'r') as f:
            mp_results[i].append(json.loads(line) for line in f)
    
    with open(output_file, 'w') as f:
        for i in range(len(input_data)):
            f.write(json.dumps(mp_results[i % num_process][i // num_process]) + '\n')
