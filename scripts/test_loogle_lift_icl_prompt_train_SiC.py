...
class ICLContextDataset(Dataset):
    ...
    def __init__(self, context: str, tokenizer: PreTrainedTokenizer, model_max_length: int=4096, block_size: int=256, len_segment: int=8, len_offset: int=3, regularization_scale: float=.0, segment_prefix: str=""):
        ...
        for s in range(0, len(input_ids), len_offset):
            ...
            result = self.prepare_lift_icl(start_pos, end_pos, input_ids, len_segment, len_offset)

            if isinstance(result, Tuple):
                lift_icl_front, lift_icl_back = result
    ...
    def prepare_lift_icl(self, start_pos: int, end_pos: int, input_ids: list[int], len_segment: int, len_offset: int, len_lift_icl: int=4096):
        """
            return one or two segment, ensuring the lift is contained in the segment.
        """
...
def LooGLEtrain(context: str, title: str, gold_qa: list, tokenizer: PreTrainedTokenizer, model_name_or_path: str, training_args: TrainingArguments, model_max_length: int=4096, block_size: int=256, len_segment: int=8, len_offset: int=3, use_lora: bool=False, lora_rank: Optional[int]=None, use_pissa: bool=False, load_in_4bit: bool=False, involve_qa_epochs: int=0, gather_batches: bool=True, num_syn_qa: int=0, title_option: int=1, generator_name_or_path: Optional[str]=None, use_gated_memory: bool=False, use_cot: bool=False, mix_training: bool=True, qa_lr: Optional[float]=None, regularization_scale: float=.0, supervise_in_context: bool=False, segment_prefix: str="", **kwargs):
    ...
    for i, sample in enumerate(dataset):
        if i >= dataset.num_segments:
            print(i - dataset.num_segments, sample['regularization_scale'])
    print('end')
...
def prediction(data: List[Dict], training_args: TrainingArguments, lift_args: Dict, output_file: str, saved_memgate: List, num_resumed: int=0, do_check_memgate: bool=False, **test_args):
    ...
    for i, sample in enumerate(tqdm.tqdm(data, desc="Sample")):
        ...
        title = sample['title']
        qa_pairs = sample['train_qa_pairs']
        model = LooGLEtrain(context, title, qa_pairs, tokenizer, training_args=training_args, **test_args, **lift_args)
        model.eval()
