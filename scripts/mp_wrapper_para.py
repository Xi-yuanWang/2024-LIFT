...
def main(args):
    ...
    if args.no_icl_output_file:
        mp_no_icl_output_files = prepare_split_filename(MP_OUTPUT_DIR, args.no_icl_output_file, args.num_test, args.num_process)
    else:
        mp_no_icl_output_files = [None for _ in range(args.num_process)]
    print('mp_output_files: ', mp_output_files)
    print('mp_no_icl_output_files: ', mp_no_icl_output_files)

    # run subprocess 
    processes = []
    for rank in range(args.num_process):
        processes.append(run_command(rank, args, mp_input_files[rank], mp_output_files[rank], mp_no_icl_output_files[rank]))
    ...
    if args.no_icl_output_file:
        collect_output(args.no_icl_output_file, mp_no_icl_output_files, args.num_process)
...
def run_command(rank: int, args, input_file, output_file, no_icl_output_file=None):
    env = os.environ.copy()
    env['CUDA_VISIBLE_DEVICES'] = str(rank)
    command = ['python', args.script] + args.subprocess_args \
                + ['--input_file', input_file, '--output_file', output_file, '--no_icl_output_file', no_icl_output_file]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_icl_output_file', type=str, required=True)
