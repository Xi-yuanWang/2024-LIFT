...
class GMLlamaAttention(LlamaAttention):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        tmp2 = GroupedLinear(self.num_heads, gatedim, 1, bias=False)
        self.cof_proj = nn.Sequential(
            GroupedLinear(self.num_heads, self.head_dim, gatedim, bias=config.attention_bias),
            nn.SiLU(inplace=True),
            tmp2,
            BiasSigmoid()
    ...
    def forward(
        ...
        **kwargs,
        ...
        mem, memgate, cof = self.mem_proj(query_states), self.gate_proj(query_states), self.cof_proj(query_states)
        ...
        attn_output = (1-memgate) * (1 - cof) * attn_output + memgate * cof * mem
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            if True: # add in context loss
                regularization_mask = regularization_scale > 0
                if torch.any(regularization_mask):
                    shifted_memgate = regularization_scale[:, None, None, None] * torch.stack(outputs.attentions, dim=1)[:, :, :, :-1, 0]  # (Batch, Layer, Head, Token)
                    shifted_memgate = shifted_memgate[regularization_mask]
                    shifted_memgate = shifted_memgate.transpose(1, 3)  # (Batch, Token, Head, Layer)
                    shifted_labels = labels[regularization_mask, 1:]
            ...
            if True: # add out context loss
                max_regularization_scale = regularization_scale.max()
                regularization_scale = max_regularization_scale - regularization_scale
                ...
                if torch.any(regularization_mask):
                    ...
                    shifted_labels = labels[regularization_mask, 1:]
                    masked_memgate = shifted_memgate[shifted_labels != -100]  # NOTE: may consider special ignore_index
                    loss_regularization = torch.mean(1 - masked_memgate)  # using the mean of memgate as regularization
                    loss += loss_regularization
